import data from "../data/reminders.json"
import bg from "../assets/backgrounds/reminders.png"
      ...
      <div className="relative z-10 mt-12 grid grid-cols-1 gap-6 max-w-4xl w-full px-8">
        {data.length > 0 ? (
          data.map((item, idx) => (
            <div
              key={idx}
              className="bg-black/40 rounded-2xl p-6 shadow-lg"
            >
          ...
          <p className="text-xl opacity-70 text-center">
            Pulled from 'Reminders' but theres nothing listed!
          </p>
        )}
      </div>
    </div>
